...
test("star rating patches in place; rating sort forces a full render", async ({ page }, ti) => {
  ...
  await card.locator(`button[data-act="rate"][data-v="2"]`).click();
  ...
  await page.locator(`.card.open button[data-act="rate"][data-v="5"]`).click();
  await expect
...
test("status change regroups the list and bumps the epoch", async ({ page }, ti) => {
  await openApp(page, ti, tinyState());
  const e0 = await epoch(page);
  const card = await openCard(page, "Beta Blade"); // backlog
  await card.locator(`button[data-act="status"][data-s="playing"]`).click();
  await expect
    .poll(async () => Number(await epoch(page)))
    .toBeGreaterThan(Number(e0));
});

test("genres edit refreshes the no-data chip without a full render", async ({ page }, ti) => {
  await openApp(page, ti, tinyState());
  ...
  await gen.fill("rpg"); // canonicalizes to RPG silently, no dialog
  await gen.blur();
...
test("un-favoriting on the Favorites tab removes the card via full render", async ({ page }, ti) => {
  await openApp(page, ti, tinyState());
  await page.locator(`.tab[data-f="fav"]`).click();
  await expect(page.locator(".card", { hasText: "Delta Drift" }).first()).toBeVisible();
  const e0 = await epoch(page);
  const card = await openCard(page, "Delta Drift");
  await card.locator(`button[data-act="fav"]`).click();
  await expect(page.locator(".card", { hasText: "Delta Drift" })).toHaveCount(0);
  expect(Number(await epoch(page))).toBeGreaterThan(Number(e0));
});
